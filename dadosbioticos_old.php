	 ...
	 {   
		exibe('loading','Buscando Ocorrências');
         ...
         else if (document.getElementById('checkfontesibbr').checked==true)
         {
			...
			var data = await getSpeciesLink(texto);
			// printSpeciesLink(data)
		 }
         else printCSV(file);
     }
	 //exibe('loading','Buscando Ocorrências');
...
function getSpeciesLink(sp)
{
	console.log('species link ' + sp);
	var xmlhttp = new XMLHttpRequest();   // new HttpRequest instance 
	var theUrl = 'http://php7.jbrj.gov.br:8080/splink';	
	...
	xmlhttp.send(JSON.stringify({"ScientificName" : sp, "Scope" : "plants", "Format": "JSON" }));
